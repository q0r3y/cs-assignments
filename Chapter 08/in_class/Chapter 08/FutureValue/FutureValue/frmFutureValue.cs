...
namespace FutureValue
{
    public partial class frmFutureValue : Form
    {
        ...
        int rowIndex = 0;
        string[,] rectangleArr = new string[10, 4];
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsValidData())
                {
                    ...
                    string[] values = {
                        monthlyInvestment.ToString("c"),
                        (interestRateYearly / 100).ToString("p"), 
                        txtYears.Text, futureValue.ToString("c") };

                    for (int i = 0; i < rectangleArr.GetLength(1); i++)
                    {
                        rectangleArr[rowIndex, i] = values[i];
                    }
        ...
        {
            string calculations = "Inv/Mo.\tRate\tYears\tFuture Value\n";


            for (int i = 0; i < rectangleArr.GetLength(0); i++)
            {
                for (int j = 0; j < rectangleArr.GetLength(1); j++)
                {
                    calculations += rectangleArr[i, j] + "\t";
                }
                calculations += "\n";
            }

            MessageBox.Show(calculations, "Future Value Calculations");
        }
